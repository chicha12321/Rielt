...
public class EdUslugaDialog extends JRDialog {
	...
	// Поля класса
	  ...
	  private final static String title_ed = 
	   ...
	   //  Объектная переменная для класса Услуга
	  ...
	  // Установка флага для режима добавления новой строки
		  ...
		  if (!isNewRow) {
		     ...
		     this.type = new Usluga();	// Новый объект
	  ...
	  //   при неправильном заполнении полей
				if (!constructUsluga())
					return;
				if (isNewRow) {
	// вызов метода менеджера Добавить новую услугу
	...
	//к компонентам окна
		...
		//Формирование объекта Услуга перед сохранением
